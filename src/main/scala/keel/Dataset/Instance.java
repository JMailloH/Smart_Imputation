...
public class Instance {
	...
	/**
	 * It is a vector of vectors of size 'number of attributes' where all the
	 * nominal values will of the attributes be stored. In nominalValues[0] the
	 * input values are stored, and int nominalValues[1] the output values are
	 * stored.
	 */
	private String[][] nominalValues;

	/**
	 * It is a vector of vector of size 'number of attributes' where all the
	 * nominal values will of the attributes be stored, but transformed to a
	 * integer value.
	 */
	private int[][] intNominalValues;

	/**
	 * The vector realValues is a vector of vectors of size 'number of
	 * attributes' where all the integer and real attributes values will be
	 * stored. In realValues[0] all the input attribute values will be stored,
	 * while in realValues[1], the outputs will be stored.
	 */
	private double[][] realValues;

	/**
	 * It is a vector of vectors of 'number of attributes' size that stores
	 * whichs attributes are missing for the inputs and the outputs.
	 */
	...
	/**
	 * Indicates the number of undefined attributes (that are neither inputs or
	 * outputs
	 */
	 ...
	 * @param _isTrain
	 *            is a flag that indicates if the BD is for a train run.
	 * @param instanceNum
	 *            is the number of the current instance. It's used to write
	 *            error message with the maximum amount of information.
	 */
	public Instance(String def, boolean _isTrain, int instanceNum) {
		int currentClass = -1;
		// System.out.println ("Reading data: "+def);
		StringTokenizer st = new StringTokenizer(def, ","); // Separator: ","
															// and " "
		...
		while (st.hasMoreTokens()) {
			...
			case Attribute.OUTPUT:
				...
				if (curAt.getType() == Attribute.NOMINAL) {
					...
					// System.out.println (
					// " The position of the current class "+ att +" is: "+
					// currentClass );
				}
				curCount = outAttCount++;
				break;
			default:// Attribute not defined neither as input or output. So, it
					// is not read.
				inputOutput = Instance.ATT_NONDEF;
				curCount = indefCount++;
			}

			// The attribute is defined. So, its value is processed, and the
			// attributes definitions
			// are checked to detect inconsistencies or to redefine undefined
			// traits.
			processReadValue(curAt, def, att, inputOutput, count, curCount,
					instanceNum);
		...
		// Checking if the instance doesn't have the same number of attributes
		// than defined.
		if (count != Attributes.getNumAttributes()) {
			ErrorInfo er = new ErrorInfo(
					ErrorInfo.BadNumberOfValues,
					instanceNum,
					InstanceParser.lineCounter,
					0,
					0,
					isTrain,
					("Instance "
							+ def
							+ " has a different number of attributes than defined\n   > Number of attributes defined: "
							+ Attributes.getNumAttributes()
							+ "   > Number of attributes read:    " + count));
			...
			for (int i = 0; i < atts.length; i++) {
				if (!missingValues[Instance.ATT_INPUT][i]) {
					if (atts[i].getType() == Attribute.NOMINAL
							&& Attributes.getOutputNumAttributes() == 1)
						atts[i].increaseClassFrequency(currentClass,
								nominalValues[Instance.ATT_INPUT][i]);
					else if ((atts[i].getType() == Attribute.INTEGER || atts[i]
							.getType() == Attribute.REAL)
							&& !missingValues[Instance.ATT_INPUT][i])
						atts[i].addInMeanValue(currentClass,
								realValues[Instance.ATT_INPUT][i]);
				}
			}
		}
	...
	public Instance(String def, boolean _isTrain, int instanceNum,
		...
		while (st.hasMoreTokens()) {
			// Looking if the attribute is an input, an output or it's undefined
			String att = st.nextToken().trim();
			Attribute curAt = atts.getAttribute(count);
			switch (curAt.getDirectionAttribute()) {
			case Attribute.INPUT:
				inputOutput = Instance.ATT_INPUT;
				curCount = inAttCount++;
		...
		// Checking if the instance doesn't have the same number of attributes
		// than defined.
		if (count != atts.getNumAttributes()) {
			ErrorInfo er = new ErrorInfo(
					ErrorInfo.BadNumberOfValues,
					instanceNum,
					InstanceParser.lineCounter,
		...
		// Compute the statistics
		if (isTrain) {
			Attribute[] atts_aux = atts.getInputAttributes();

			for (int i = 0; i < atts_aux.length; i++) {
				if (!missingValues[Instance.ATT_INPUT][i]) {
					if (atts_aux[i].getType() == Attribute.NOMINAL
							&& atts.getOutputNumAttributes() == 1)
						atts_aux[i].increaseClassFrequency(currentClass,
								nominalValues[Instance.ATT_INPUT][i]);
					else if ((atts_aux[i].getType() == Attribute.INTEGER || atts_aux[i]
							.getType() == Attribute.REAL)
							&& !missingValues[Instance.ATT_INPUT][i])
						atts_aux[i].addInMeanValue(currentClass,
								realValues[Instance.ATT_INPUT][i]);
				}
			}
		}
	...
	public Instance(Instance inst) {
		...
		this.anyMissingValue = Arrays.copyOf(inst.anyMissingValue,
				inst.anyMissingValue.length);

		this.nominalValues = new String[inst.nominalValues.length][];
		for (int i = 0; i < nominalValues.length; i++) {
			this.nominalValues[i] = Arrays.copyOf(inst.nominalValues[i],
					inst.nominalValues[i].length);
		}

		this.intNominalValues = new int[inst.intNominalValues.length][];
		for (int i = 0; i < nominalValues.length; i++) {
			this.intNominalValues[i] = Arrays.copyOf(inst.intNominalValues[i],
					inst.intNominalValues[i].length);
		}

		this.realValues = new double[inst.realValues.length][];
		for (int i = 0; i < realValues.length; i++) {
			this.realValues[i] = Arrays.copyOf(inst.realValues[i],
					inst.realValues[i].length);
		}

		this.missingValues = new boolean[inst.missingValues.length][];
		for (int i = 0; i < missingValues.length; i++) {
			this.missingValues[i] = Arrays.copyOf(inst.missingValues[i],
					inst.missingValues[i].length);
		}
	}

	/**
	 * Creates an instance from a set of given values. It is supposed that the
	 * values correspond to the current Attributes static definition or
	 * InstanceAttributes non-static definition (it depends on the InstanceSet
	 * to which this new instance will belong). If ats is null, we will use the
	 * Attributes static definiton. If not the ats definition instead.
	 * 
	 * @param values
	 *            A double array with the values (either real or nominals'
	 *            index). Missing values are stored as Double.NaN
	 * @param ats
	 *            The definition of the attributes (optional, if null we use
	...
	public Instance(double values[], InstanceAttributes ats) {
		...
		if (ats == null) {
			...
			numUndefinedAttributes = Attributes.getNumAttributes()
					- (numInputAttributes + numOutputAttributes);
		} else {
			numInputAttributes = ats.getInputNumAttributes();
			numOutputAttributes = ats.getOutputNumAttributes();
			numUndefinedAttributes = ats.getNumAttributes()
					- (numInputAttributes + numOutputAttributes);
		}
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (new Double(values[i]).isNaN()) {
			...
			} else if (!(curAt.getType() == Attribute.NOMINAL)) { // is
																	// numerical?
				if (inOut == 0) {
				...
				if (inOut == 0) {
					...
					realValues[inOut][in] = values[i];
					nominalValues[inOut][in] = curAt
							.getNominalValue((int) values[i]);
					in++;
				} else if (inOut == 1) {
					intNominalValues[inOut][out] = (int) values[i];
					realValues[inOut][out] = values[i];
					nominalValues[inOut][out] = curAt
							.getNominalValue((int) values[i]);
					out++;
				} else {
					intNominalValues[inOut][undef] = (int) values[i];
					realValues[inOut][undef] = values[i];
					nominalValues[inOut][undef] = curAt
							.getNominalValue((int) values[i]);
					undef++;
				}
			}
	 ...
	 * 
	 * @param curAtt
	 *            is the current attribute (the value read is from this
	 *            attribute)
	 * @param def
	 *            is the whole String
	 * @param inOut
	 *            is an integer that indicates if the attribute is an input or
	 *            an output attribute
	 * @param count
	 *            is a counter of attributes.
	 * @param curCount
	 *            is an attribute counter relative to the inputs or the output.
	 *            So, it indicates that the attribute is the ith attribute of
	 *            the input or the output.
	 * @param instanceNum
	 *            is the number of the current instance. It's needed to write
	 *            output messages with the maximum possible amount of
	 *            information.
	 */
	private void processReadValue(Attribute curAtt, String def, String att,
			int inOut, int count, int curCount, int instanceNum) {
		...
		if (att.equalsIgnoreCase("<null>") || att.equalsIgnoreCase("?")) {
			...
			anyMissingValue[inOut] = true;
			if (inOut == 1) { // If the output is a missing value, an error is
								// generated.
				ErrorInfo er = new ErrorInfo(ErrorInfo.OutputMissingValue,
						instanceNum, InstanceParser.lineCounter, curCount,
						Attribute.OUTPUT, isTrain, ("Output attribute " + count
								+ " of " + def + " with missing value."));
				InstanceSet.errorLogger.setError(er);
			}
			...
			try {
				...
				ErrorInfo er = new ErrorInfo(
						ErrorInfo.BadNumericValue,
						instanceNum,
						InstanceParser.lineCounter,
						curCount,
						Attribute.INPUT + inOut,
						isTrain,
						("Attribute " + count + " of " + def + " is not an integer or real value."));
				InstanceSet.errorLogger.setError(er);
			}
			// Checking if the new train value exceedes the bounds definition in
			// train. The condition
			// also checks if the attribute is defined (is an input or an
			// output).
			if (isTrain && inOut != 2) {
				if (curAtt.getFixedBounds()
						&& !curAtt.isInBounds(realValues[inOut][curCount])) {
					ErrorInfo er = new ErrorInfo(
							ErrorInfo.TrainNumberOutOfRange,
							instanceNum,
							InstanceParser.lineCounter,
							curCount,
							Attribute.INPUT + inOut,
							...
							("ERROR READING TRAIN FILE. Value "
									+ realValues[inOut][curCount]
									+ " read for a numeric attribute that is not in the bounds fixed in the attribute '"
									+ curAtt.getName() + "' definition."));
					InstanceSet.errorLogger.setError(er);
				}
				curAtt.enlargeBounds(realValues[inOut][curCount]);
			} else if (inOut != 2) { // In test mode
				realValues[inOut][curCount] = curAtt
						.rectifyValueInBounds(realValues[inOut][curCount]);
			}
			...
			if (isTrain && inOut != 2) {
				if (curAtt.getFixedBounds()
						&& !curAtt
								.isNominalValue(nominalValues[inOut][curCount])) {
					ErrorInfo er = new ErrorInfo(
							ErrorInfo.TrainNominalOutOfRange,
							instanceNum,
							InstanceParser.lineCounter,
							curCount,
							Attribute.INPUT + inOut,
							isTrain,
							("ERROR READING TRAIN FILE. Value '"
									+ nominalValues[inOut][curCount]
									+ "' read for a nominal attribute that is not in the possible list of values fixed in the attribute '"
									+ curAtt.getName() + "' definition."));
					InstanceSet.errorLogger.setError(er);
				}
				...
				if (curAtt.addTestNominalValue(nominalValues[inOut][curCount])) {
					ErrorInfo er = new ErrorInfo(
							ErrorInfo.TestNominalOutOfRange,
							instanceNum,
							InstanceParser.lineCounter,
							curCount,
							Attribute.INPUT + inOut,
							isTrain,
							("ERROR READING TEST FILE. Value '"
									+ nominalValues[inOut][curCount]
									+ "' read for a nominal attribute that is not in the possible list of values fixed in the attribute '"
									+ curAtt.getName() + "' definition."));
					InstanceSet.errorLogger.setError(er);
			...
			if (inOut != -2) {
				intNominalValues[inOut][curCount] = curAtt
						.convertNominalValue(nominalValues[inOut][curCount]);
	...
	private void processReadValue(Attribute curAtt, String def, String att,
		...
		} else if (atts.getAttribute(count).getType() == Attribute.INTEGER
				|| atts.getAttribute(count).getType() == Attribute.REAL) {
			try {
				realValues[inOut][curCount] = Double.parseDouble(att);
			} catch (NumberFormatException e) {
				ErrorInfo er = new ErrorInfo(
		...
		} else if (atts.getAttribute(count).getType() == Attribute.NOMINAL) {
			nominalValues[inOut][curCount] = att;
			// Testing special cases.
			if (isTrain && inOut != 2) {
				if (curAtt.getFixedBounds()
	 ...
	 * It reserves all the memory necessary for this instance
	 */
	private void initClassAttributes(InstanceAttributes atts) {
		anyMissingValue = new boolean[3];
		anyMissingValue[0] = false;
		anyMissingValue[1] = false;
		anyMissingValue[2] = false;
		numInputAttributes = atts.getInputNumAttributes();
		numOutputAttributes = atts.getOutputNumAttributes();
		numUndefinedAttributes = atts.getNumAttributes()
				- (numInputAttributes + numOutputAttributes);
		intNominalValues = new int[3][];
		nominalValues = new String[3][];
		realValues = new double[3][];
	...
	/**
	 * It prints the instance to the specified PrintWriter. The attribtes order
	 * is the same as the one in the original file.
	 * 
	 * @param out
	 *            is the PrintWriter where to print.
	 */
	public void printAsOriginal(PrintWriter out) {
		...
		for (count = 0; count < numAttributes; count++) {
			...
			case Attribute.DIR_NOT_DEF:
				printAttribute(out, Instance.ATT_NONDEF, undefCount,
						at.getType());
				undefCount++;
				break;
			}
	 ...
	 * 
	 * @return a double[] of size equal to the number of input attributes with
	 *         the values of real attributes. Positions of the vector that
	 *         doesn't correspond to a real attribute has no rellevant data.
	 */
	 ...
	 * 
	 * @return a string[] of size equal to the number of input attributes with
	 *         the values of nominal attributes. Positions of the vector that
	 ...
	 * 
	 * @return a boolean[] of size equal to the number of input attributes. A
	 *         true value in the position i of a vector indicates that the ith
	 ...
	 * 
	 * @return a double[] of size equal to the number of output attributes with
	 *         the values of real attributes. Positions of the vector that
	 *         doesn't correspond to a real attribute has no rellevant data.
	 */
	 ...
	 * 
	 * @return a string[] of size equal to the number of Output attributes with
	 *         the values of nominal attributes. Positions of the vector that
	 *         doesn't correspond to a nominal attribute has no rellevant data.
	 */
	 ...
	 * 
	 * @return a boolean[] of size equal to the number of Output attributes. A
	 *         true value in the position i of a vector indicates that the ith
	...
	/**
	 * It does return all the input values. Doesn't care the type of the
	 * attributes. Nominal attributes are transformed to an integer, that is
	 * codified with a double. And integer attributes are codified with a double
	 * to. So all the values are returnes as doubles.
	 * 
	...
	/**
	 * It does return the normalized values in a double[]. It means that
	 * integers are normalized o [0..N], reals to [0..1] and nominals are
	 * transformed to an integer value between [0..N], where N is the number of
	 * values that this nominal can take. In addition, missing values are
	 * represented with a -1 value.
	 */
	public double[] getNormalizedInputValues() {
		double[] norm = new double[realValues[0].length];
		for (int i = 0; i < norm.length; i++) {
			if (!missingValues[0][i])
				norm[i] = Attributes.getInputAttribute(i).normalizeValue(
						realValues[0][i]);
			else
				norm[i] = -1.;
		}
	...
	/**
	 * It does return all the output values. Doesn't care the type of the
	 * attributes. Nominal attributes are transformed to an integer, that is
	 * codified with a double. And integer attributes are codified with a double
	 * to. So all the values are returnes as doubles.
	 * 
	 ...
	 * 
	 * @param pos
	 *            is the attribute that has to be changed
	 * @param value
	 *            is the new value
	 ...
	 * 
	 * @param pos
	 *            is the position of the attribute.
	 * @param value
	 *            is the new value.
	 ...
	 * 
	 * @param attToDel
	 *            is a reference to the attribute to be deleted.
	 * @param inputAtt
	 *            is a boolean that indicates if the attribute to be removed is
	 *            an input attribute (otherwise is an output attribute)
	 * @param whichAtt
	 *            is the position of the attribute to be deleted.
	 */
	void removeAttribute(Attribute attToDel, boolean inputAtt, int whichAtt) {
		...
		// Copying the new vectors without the information of the removed
		// attribute.
	 ...
	 * attributes that are defined as inputs or outputs. So,
	 * NON-SPECIFIED-DIRECTION attributes are not included to this string. The
	 * order followed is: first, all input attributes are writen, in the order
	 * in which they have been read. After that, the output attributes are
	 * write. This can alter the initial order, but never mind if the output
	 * writen has the inputs and outputs correctly defined.
	 * 
	...
	public String toString() {
		...
		String ending = ",";
		for (int i = 0; i < numInputAttributes; i++) {
			if (i == numInputAttributes - 1 && numOutputAttributes == 0)
				ending = "";
			switch (Attributes.getInputAttribute(i).getType()) {
			case Attribute.NOMINAL:
	 ...
	 *            is the PrintWriter where to print.
	 */
	public void printAsOriginal(InstanceAttributes instAttributes,
			PrintWriter out) {
		int inCount = 0, outCount = 0, undefCount = 0, count;
	...
	/**
	 * Obtains the normalized input attributes from a InstanceAttribute
	 * definition
	 * 
	 * @param instAttributes
	 *            The Attributes definition needed to normalize
	...
	public double[] getNormalizedInputValues(InstanceAttributes instAttributes) {
		...
		for (int i = 0; i < norm.length; i++) {
			if (!missingValues[0][i])
				norm[i] = instAttributes.getInputAttribute(i).normalizeValue(
						realValues[0][i]);
			else
				norm[i] = -1.;
		}
	...
	/**
	 * Obtains the normalized output attributes from a InstanceAttribute
	 * definition
	 * 
	 * @param instAttributes
	 *            The Attributes definition needed to normalize
	...
	public double[] getNormalizedOutputValues(InstanceAttributes instAttributes) {
		...
		for (int i = 0; i < norm.length; i++) {
			if (!missingValues[1][i])
				norm[i] = instAttributes.getOutputAttribute(i).normalizeValue(
						realValues[1][i]);
			else
				norm[i] = -1.;
		}
	...
	/**
	 * Set a new value of a given input attribute in this instance (integer or
	 * real)
	 * 
	 * @param instAttributes
	 *            The Attributes reference definition
	 * @param pos
	 *            The position of the input attribute to be changed in
	 *            instAttributes
	 * @param value
	 *            The new value
	 * @return true if succeeded, false otherwise
	 */
	public boolean setInputNumericValue(InstanceAttributes instAttributes,
	...
	/**
	 * Set a new value of a given output attribute in this instance (integer or
	 * real)
	 * 
	 * @param instAttributes
	 *            The Attributes reference definition
	 * @param pos
	 *            The position of the output attribute to be changed in
	 *            instAttributes
	 * @param value
	 *            The new value
	 * @return true if succeeded, false otherwise
	 */
	public boolean setOutputNumericValue(InstanceAttributes instAttributes,
			int pos, double value) {
	 ...
	 * @return true if succeeded, false otherwise
	 */
	public boolean setInputNominalValue(InstanceAttributes instAttributes,
	 ...
	 * @return true if succeeded, false otherwise
	 */
	public boolean setOutputNominalValue(InstanceAttributes instAttributes,
			int pos, String value) {
		Attribute at = (Attribute) instAttributes.getOutputAttribute(pos);
		if (at.getType() != Attribute.NOMINAL)
			return false;
	...
	void removeAttribute(InstanceAttributes instAttributes, Attribute attToDel,
			boolean inputAtt, int whichAtt) {
		int newSize;
	...
	/**
	 * Prints the instance in KEEL format, according to the given Attributes
	 * definition
	 * 
	 * @param instAttributes
