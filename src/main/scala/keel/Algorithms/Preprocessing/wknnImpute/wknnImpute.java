...
/**
 * <p>
 * @author Written by Juli�n Luengo Mart�n 05/06/2006
 * @version 0.3
 * @since JDK 1.5
 * </p>
 */
package keel.Algorithms.Preprocessing.wknnImpute;
import java.io.*;
import keel.Dataset.*;
...
public class wknnImpute {
     ...
     * @param fileParam The path to the configuration file with all the parameters in KEEL format
     */
    public wknnImpute(String input_tra, String input_tst, String output_tra, String output_tst, int neighbors) {
    ...
    private void config_read(String fileParam){
        ...
        try {
            ...
            out = line.split("k = ");
            nneigh = (new Integer(out[1])).intValue(); //parse the string into a integer
            
            do{
                line = buf_reader.readLine();
            }while(line.length()==0);
            out = line.split("enn = ");
            ennNeighbors = (new Integer(out[1])).intValue(); //parse the string into a integer
            
            do{
                line = buf_reader.readLine();
            }while(line.length()==0);
            out = line.split("eta = ");
            cleanThreshold = (new Double(out[1])).doubleValue(); //parse the string into a double
     ...
     */
    protected double distance(Instance i,Instance j){
        double dist = 0;
        int in = 0;
        int out = 0;
        ...
        for(int l = 0; l < nvariables;l++){
            ...
            if(direccion == Attribute.INPUT){
                if(tipo != Attribute.NOMINAL && !i.getInputMissingValues(in)){
                    ...
                    dist += ( (i.getInputRealValues(in)-j.getInputRealValues(in))/range ) * ( (i.getInputRealValues(in)-j.getInputRealValues(in)) / range );
                } else{
                	...
                	if(direccion == Attribute.INPUT){
                		if(tipo != Attribute.NOMINAL && !inst.getInputMissingValues(in)){
                        	if (tipo == Attribute.INTEGER)
								X[i][j] = new String(String.valueOf((int) inst.getInputRealValues(in)));
							else
                            ...
                            if(!inst.getInputMissingValues(in))
                            	...
                            	//TODO - avoid to impute filtered instances to save computing time
                            	allNull = true;
                                ...
                                if(tipo != Attribute.NOMINAL){
                                    	...
                                    	if(N[m]!=-1){
                                    		...
                                    		if(!inst2.getInputMissingValues(in)){
                                    			if(totalDist != 0){
                                    				...
                                    				mean += inst2.getInputRealValues(in)*(1.0/nonNullNeighs);
                                    			}
                                    			allNull = false;
                                    		}
                                    	}
                                    ...
                                    if(!allNull){
                                    	...
                                    	if(tipo == Attribute.INTEGER)
                                    		X[i][j] = new String(String.valueOf((int)  mean ));
                                    	else
                        ...
                        if(direccion == Attribute.OUTPUT){
                            if(tipo != Attribute.NOMINAL && !inst.getOutputMissingValues(out)){
                            	if (tipo == Attribute.INTEGER)
    								X[i][j] = new String(String.valueOf((int) inst.getInputRealValues(out)));
    							else
    								X[i][j] = new String(String.valueOf(inst.getInputRealValues(out)));
                            } else{
